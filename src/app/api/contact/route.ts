...
export async function POST(req: Request) {
  try {
    const { name, email, message, track, website } = await req.json()
    if (website) return NextResponse.json({ ok: true }) // honeypot
    if (!name || !email || !message) return NextResponse.json({ error: "bad_request" }, { status: 400 })
    ...
    if ((r as any).error) return NextResponse.json({ error: "send_failed" }, { status: 500 })
