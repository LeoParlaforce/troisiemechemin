...
const PACK_ARCHIVE = "Guides psychologiques - Pack intégral.rar"
...
function normalizeSlug(raw: string): { norm: string; isPack: boolean } {
  ...
  return { norm: isPack ? PACK_SLUG : slug, isPack }
}

export async function GET(req: Request) {
  try {
    const url = new URL(req.url)
    const { norm: slug, isPack } = normalizeSlug(url.searchParams.get("slug") || "")
    ...
    if (!s || s.payment_status !== "paid") {
      return NextResponse.json({ error: "stripe_session_invalid" }, { status: 403 })
    }
    ...
    return NextResponse.json({ error: "download_error" }, { status: 500 })
  }
}