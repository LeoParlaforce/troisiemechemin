...
export default function SuccessClient() {
  ...
  const [slug, setSlug] = useState<string | null>(null)       // <- pour l’e-book
  ...
  const gcal = useMemo(() => {
    ...
    return gcalUrl({
      ...
      location: "Jitsi Meet", ctz: "Europe/Paris", recur: "FREQ=WEEKLY;INTERVAL=2",
    })
  }, [track])
  ...
  return (
    <main className="max-w-5xl mx-auto px-6 py-16 text-foreground">
      ...
      {email && <p className="mt-2 opacity-80">Confirmation pour <span className="font-medium">{email}</span>.</p>}
      ...
      {/* E-book: bouton de téléchargement immédiat */}
      {!track && slug && (
        <div className="mt-8">
          <a
            href={`/boutique/${slug}`}   // si tu as un endpoint /api/download/<slug>, mets-le ici
      ...
      {/* Tarif membre visible seulement pour les inscrits aux groupes */}
      {isMember && (
