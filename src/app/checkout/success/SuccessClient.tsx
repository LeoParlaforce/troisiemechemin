...
export default function SuccessClient() {
  ...
  const discounted: DiscountProduct[] = useMemo(() => {
    const list: DiscountProduct[] = products.map(p => ({ slug: p.slug, title: p.title, image: p.image }))
    if (!list.some(p => p.slug === "pack-integral")) list.unshift({ slug: "pack-integral", title: "Pack intégral" })
    return list
  }, [])
    ...
    <main className="max-w-5xl mx-auto px-6 py-16 text-foreground">
      ...
      <section className="mt-12">
        ...
        <p className="mt-1 text-sm opacity-70">
          Avec votre inscription à un groupe, chaque guide est à <strong>5 €</strong> (au lieu de 9 €). Le pack intégral est à <strong>29 €</strong> (au lieu de 49 €).
        </p>
